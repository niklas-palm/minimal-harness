...
function htmlToText(html: string): string {
  ...
  const text = html
    ...
    .replace(/<!--[\s\S]*?-->/g, " ")
    .replace(
      /<\/(p|div|section|article|li|h[1-6]|tr|pre|blockquote)>|<br\s*\/?>/gi,
      "\n",
    )
    .replace(/<[^>]+>/g, " ")
    .replace(/&nbsp;/g, " ")
...
async function fetchText(url: string) {
  const res = await fetch(url, {
    headers: {
      "user-agent":
        "harness/0.2 (+https://github.com/niklas-palm/minimal-harness)",
    },
...
async function search(
  ...
  if (!res.ok)
    throw new Error(
      `gateway returned ${res.status}: ${(await res.text()).slice(0, 300)}`,
    );
